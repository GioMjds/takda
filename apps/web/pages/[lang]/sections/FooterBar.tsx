...
export function FooterBar({ lang }: FooterBarProps) {
  ...
  return (
    <footer className="border-t border-white/[0.06] bg-[#0a1f1a]/40 backdrop-blur-sm">
      <div className="mx-auto flex w-full max-w-6xl flex-col items-center justify-between gap-3 px-6 py-6 sm:flex-row sm:gap-0 sm:px-10">
        ...
        >
          <span className="font-[family-name:var(--font-display)] text-base font-light tracking-tight text-[#e8f5ef]/55 transition-colors group-hover:text-[#e8f5ef]">
            Takda
          </span>
          <span
            aria-hidden="true"
...
function FooterLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link
      href={href as Route}
      className="rounded px-2 py-1 transition-colors hover:text-[#e8f5ef] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1D9E75]/60"
    >
      {children}
    </Link>
  );
